...
// 🔑 Importa gli hook e le azioni Redux
import { useAppSelector, useAppDispatch } from '../store/store'; 
import { setOffer, selectCartTotal, syncCart, CartState } from '../store/cartSlice'; 
import { Ionicons } from '@expo/vector-icons';
// 🔑 Importazioni necessarie per Mappa e GPS
...
export default function CheckoutScreen() {
  ...
  // 🔑 Seleziona lo stato del carrello e calcola il totale
  const cartState = useAppSelector(state => state.cart);
  const offer = cartState.offer;
  const isSyncing = cartState.isSyncing;
  const syncError = cartState.syncError;
  const total = useAppSelector(state => selectCartTotal(state.cart));
  // 🔑 Sincronizzazione con il backend mock ad ogni cambio nel carrello
  useEffect(() => {
    // La sincronizzazione viene lanciata ad ogni modifica del carrello
    dispatch(syncCart(cartState as CartState));
  }, [cartState.items, cartState.offer]);
        ...
        <View style={styles.syncStatusContainer}>
            {isSyncing && (
                <View style={styles.syncRow}>
                    ...
                    <Text style={styles.syncText}>Sincronizzazione in corso...</Text>
                </View>
            )}
            {syncError && (
                <Text style={styles.errorText}>Errore sync: {syncError}</Text>
            )}
            {cartState.lastSync && !isSyncing && !syncError && (
                <Text style={styles.lastSyncText}>Ultima sincronizzazione: {new Date(cartState.lastSync).toLocaleTimeString()}</Text>
            )}
        </View>

        <View style={styles.totalRow}>
            <Text style={styles.totalText}>Totale:</Text>
            <Text style={[styles.totalText, styles.totalValue]}>{total.toFixed(2)} €</Text>
        </View>
